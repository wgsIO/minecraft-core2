...
public class ShortAdapter implements Adapter<Short> {
    ...
    @Override
    public Short adapt(CommandSender sender, ParameterInfo parameterInfo, String value) {
        return Optional.ofNullable(value).map(($) -> NumberQuery.from($).shortValue()).orElse(null);
    }
