...
public class DoubleAdapter implements Adapter<Double> {
    ...
    @Override
    public Double adapt(CommandSender sender, ParameterInfo parameterInfo, String value) {
        return Optional.ofNullable(value).map(($) -> NumberQuery.from($).doubleValue()).orElse(null);
    }
