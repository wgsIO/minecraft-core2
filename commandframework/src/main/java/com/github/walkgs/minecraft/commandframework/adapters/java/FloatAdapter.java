...
public class FloatAdapter implements Adapter<Float> {
    ...
    @Override
    public Float adapt(CommandSender sender, ParameterInfo parameterInfo, String value) {
        return Optional.ofNullable(value).map(($) -> NumberQuery.from($).floatValue()).orElse(null);
    }
