...
public class LongAdapter implements Adapter<Long> {
    ...
    @Override
    public Long adapt(CommandSender sender, ParameterInfo parameterInfo, String value) {
        return Optional.ofNullable(value).map(($) -> NumberQuery.from($).longValue()).orElse(null);
    }
